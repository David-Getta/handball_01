...
def _coach_keys(rep: ScoutingReport) -> tuple[list, list, list]:
    ...
    # Eltűnő ember: az első félidőben kell megfogni.
    for _fdr_p in (rep.fdr_players or []):
        if _fdr_p["fh"] >= 3 and _fdr_p["fh"] >= 3 * max(1, _fdr_p["sh"]):
            keys.append(
                f"A(z) {_fdr_p['player_id']}. számú emberük az első "
                f"félidőben él ({_fdr_p['fh']} gól-részvétel), a "
                f"másodikban eltűnik ({_fdr_p['sh']}) — az első 30 "
    ...
    # Felzárkózás-húzó: a mentőemberük kivehető a játékból.
    for _cbc_p in (rep.cbc_players or []):
        if (_cbc_p["trailing"] >= 3
                and _cbc_p["trailing"] >= 2 * max(1, _cbc_p["rest"])):
            keys.append(
                f"A(z) {_cbc_p['player_id']}. számú játékosuk hozza "
                f"őket vissza hátrányból ({_cbc_p['trailing']} "
                f"gól-részvétel hátrányban, máskor {_cbc_p['rest']}) "
                "— ha vezettek, őt vegyétek ki a játékból (szoros "
    ...
    if _rht_h >= 16:
        _rht_avg = (sum(rep.rht_frames_by_role.values()) / _rht_h / 25.0)
        _rht_rows = [(p, h, rep.rht_frames_by_role.get(p, 0) / h / 25.0)
                     for p, h in rep.rht_holds_by_role.items() if h >= 8]
        if _rht_rows:
            _rht_p, _rht_n, _rht_s = max(_rht_rows, key=lambda r: r[2])
            if _rht_s - _rht_avg >= 0.7:
                keys.append(
                    f"A(z) {_rht_p} posztjuknál áll meg a labda: "
                    f"{_rht_s:.1f} mp/érintés a csapat-átlag "
                    f"{_rht_avg:.1f} mp helyett ({_rht_n} labdás "
                    "szakaszból) — ő a kettőzés célpontja, nála van idő "
                    "odaérni.")
...
def _scout_team_cached(match: Match, team: Team,
    ...
    try:
        ...
        rep.blw_conceded = {b: v["conceded"]
                            for b, v in blwrec["buckets"].items()
                            if v["conceded"]}
        from .momentum import fading_scorers as _fdr
        rep.fdr_players = [
        ...
        rep.fdd_players = [
            ...
            if p["fh"] or p["sh"]]
        from .momentum import comeback_carriers as _cbc
        rep.cbc_players = [
        ...
        rep.rht_holds_by_role = {p: r["holds"]
                                 for p, r in rhtrec["roles"].items()}
        # KOCKA-összeg (nem átlag): meccsek közt pontosan összegződik.
        rep.rht_frames_by_role = {
            p: round(r["holds"] * r["avg_s"] * 25.0)
            for p, r in rhtrec["roles"].items()}
...
def _merge_cbc_players(reports) -> list:
    """Játékosonkénti hátrány/máskor gól-részvételek pontos összegzése."""
    tally: dict = {}
    for r in reports:
        for e in (r.cbc_players or []):
            rec = tally.setdefault(e["player_id"], [0, 0])
            rec[0] += int(e["trailing"])
            rec[1] += int(e["rest"])
    return [{"player_id": pid, "trailing": tr, "rest": rest}
            for pid, (tr, rest) in sorted(tally.items(),
                                          key=lambda kv: -kv[1][0])]
...
def _merge_fdr_players(reports) -> list:
    """Játékosonkénti félidei gól-részvételek pontos összegzése."""
    tally: dict = {}
    for r in reports:
        for e in (r.fdr_players or []):
            rec = tally.setdefault(e["player_id"], [0, 0])
            rec[0] += int(e["fh"])
            rec[1] += int(e["sh"])
    return [{"player_id": pid, "fh": fh, "sh": sh}
...
def matchup_plan(own: "ScoutingReport",
    ...
    _rht261_h = sum(opp.rht_holds_by_role.values())
    if _rht261_h >= 16:
        _rht261_avg = (sum(opp.rht_frames_by_role.values())
                       / _rht261_h / 25.0)
        _rht261_rows = [(p, h, opp.rht_frames_by_role.get(p, 0) / h / 25.0)
                        for p, h in opp.rht_holds_by_role.items() if h >= 8]
        if _rht261_rows:
            _p261, _n261, _s261 = max(_rht261_rows, key=lambda r: r[2])
            if (_s261 - _rht261_avg >= 0.7
                    and own.dbl_doubled_frames >= 50):
                plan.append(
                    f"A(z) {_p261} posztjuknál áll meg a labda "
                    f"({_s261:.1f} mp/érintés a csapat-átlag "
                    f"{_rht261_avg:.1f} mp helyett), ti pedig tudtok "
                    "kettőzni — a második ember ODA menjen: nála van "
                    "idő odaérni, és ott lassul a támadásuk amúgy is.")
    ...
    _cbc246 = next(
        (p for p in (opp.cbc_players or [])
         if p["trailing"] >= 3
         and p["trailing"] >= 2 * max(1, p["rest"])), None)
    if _cbc246 is not None and own.suspensions <= 2:
        plan.append(
            f"Hátrányból a(z) {_cbc246['player_id']}. számú játékosuk "
            f"hozza őket vissza ({_cbc246['trailing']} gól-részvétel "
            f"hátrányban), ti pedig kiállítás nélkül tudtok keményen "
            f"fogni ({own.suspensions} kiállítás) — ha megvan a "
            "vezetés, őt vegyétek ki a játékból szoros fogással és "
            "korai kettőzéssel: a mentés az ő kezében van, a többiek "
    ...
    _fdr243 = next(
        (p for p in (opp.fdr_players or [])
         if p["fh"] >= 3 and p["fh"] >= 3 * max(1, p["sh"])), None)
    if _fdr243 is not None and own.suspensions <= 2:
        plan.append(
            f"A(z) {_fdr243['player_id']}. számú emberük az első "
            f"félidőben él ({_fdr243['fh']} gól-részvétel), aztán "
            f"eltűnik ({_fdr243['sh']}), ti pedig kiállítás nélkül "
            f"tudtok keményen védekezni ({own.suspensions} "
            "kiállítás) — az első 30 percre tegyétek rá a legjobb "
            "őrzőtöket friss lábbal: ha az elejét megfogjátok, a "
            "második félidő ellene magától megoldódik.")
...
def combine_reports(reports: list[ScoutingReport]) -> ScoutingReport:
    ...
    rep = ScoutingReport(
        ...
        rht_frames_by_role=_merge_count_dicts(
            r.rht_frames_by_role for r in reports),
