...
Összesen **509 réteg**, modulonként csoportosítva; a
...
| `backward_passers` | Hátrapasszolók: KINÉL FORDUL VISSZA a játék. |
...
| `breakthrough_yield` | Áttörés-hozam: BEJUTNAK-E a falba, és BÜNTETNEK-E onnan. |
...
| `pivot_usage_fade` | Beálló-bevonás esése: ELJUT-E MÉG A LABDA a beállóhoz a hajrában. |
...
| `wing_involvement_fade` | Szélső-bevonás esése: ELJUT-E MÉG A LABDA a szélre a hajrában. |
...
| `press_outlets` | Menekülők: NYOMÁS ALATT KIHEZ megy a labda. |
...
| `soft_passers` | Lágy passzolók: KINEK a labdáiba lehet belenyúlni. |
...
| `defense_shapes` | Fal-alak: HOGYAN ÁLL A FALUK valójában — alak szerint, hozammal. |
...
| `defensive_gaps` | Fal-rés térkép: HOL és MEKKORA a legnagyobb rés a falban. |
...
| `gap_fade` | Fal-rés fáradás: SZÉTNYÍLNAK-E a közök a második félidőre. |
...
| `retreat_fade` | Visszaállás-fáradás: LASSUL-E a hazaérés a második félidőre. |
...
| `pre_assist_roles` | Rejtett szervező poszt: MELYIK POSZTON fut a másod-előkészítés. |
| `pre_assists` | Hoki-assziszt: KI adja a gólpassz ELŐTTI passzt. |
...
| `gk_change_yield` | Kapuscsere-hozam: FORDÍT-E a kapuscseréjük. |
...
| `outlet_targets` | Felhozatal-emberek: KIRE hozzák fel a labdát a kaputól. |
...
| `control_timeline` | Kontroll-idővonal: KI DIKTÁLT ötpercenként. |
...
| `lead_scorers` | Előnyben-emberek: KI viszi a játékot vezetésnél. |
...
| `response_scorers` | Válaszoló emberek: KAPOTT GÓL UTÁN ki válaszol. |
| `response_turnover_players` | Válaszhiba-emberek: KAPOTT GÓL UTÁN ki veszíti el a labdát. |
...
| `restart_yield` | Középkezdés-hozam: GÓLRA VÁLTJÁK-E a gól utáni újraindítást. |
...
| `second_start_scorers` | Újrakezdő emberek: KI viszi a szünet utáni rajtot. |
| `super_sub` | Szuper-csere: KI termel a padról — névre szólóan. |
| `super_sub_roles` | Szuper-csere poszt: MELYIK POSZTRÓL termel a paduk. |
...
| `assisted_scorers` | Kiszolgált befejezők: KI él a bejátszásokból. |
...
| `role_shooting_hand` | Poszt-kezesség: MELYIK POSZTJUKON lő balkezes. |
...
| `passive_holders` | Passzív-birtoklók: KINÉL hal el a felállt támadásuk. |
| `passive_risk` | Passzív-kockázat: MENNYIRE gyakran futnak bele a passzív jelbe. |
...
| `powerplay_turnover_players` | Emberelőny-hibázók: KI ADJA EL a labdát a két perc alatt. |
| `powerplay_turnover_roles` | Emberelőny-hiba poszt: KINEK A KEZÉN akad el az emberelőnyük. |
| `powerplay_yield` | Emberelőny-hozam: MEGBÜNTETIK-E a kiállítást. |
...
| `seven_taker_corners` | Hetes-sarok emberre: MELYIK SARKÁT keresi a hetesdobójuk. |
| `seven_taker_players` | Hetesdobók: KI ÁLL ODA a hétméteresekhez. |
| `seven_taker_repeat` | Hetes-ismétlés: MÁSODSZORRA IS UGYANODA megy-e a hetesük. |
...
| `shorthanded_survival` | Emberhátrány-túlélés: MIT ÉR ellenük az emberelőny. |
| `shorthanded_turnover_players` | Emberhátrány-hibázók: ÖT EMBERREL ki veszíti el a labdát. |
...
| `suspension_collectors` | Kétperc-gyűjtők: KI ül ki náluk a legtöbbször. |
...
| `setplay_repeat_choice` | Figura-ismétlés döntése: GÓL UTÁN UGYANAZT hozzák-e újra. |
| `setplay_shapes` | Figura-alakok: a meccsen felismert figurák ALAKJA + darabszámai — |
...
| `iron_men` | Vasemberek: KI játssza végig a meccset csere nélkül. |
...
| `running_load_balance` | Futómunka-eloszlás: HÁNY EMBERRE épül a futómunkájuk. |
...
| `timeout_turnover_players` | Időkérés-hibázók: A MEGBESZÉLT FIGURA kinek a kezén hal el. |
...
| `big_chance_feeders` | Ziccer-előkészítők: KI ADJA a passzt a nagy helyzethez. |
...
| `missed_chance_players` | Ziccerhagyó emberek: KI hagyja ki a ziccereket. |
